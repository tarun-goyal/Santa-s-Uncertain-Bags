...
def _fill_bags_with_gifts():
    bags = {}
    toys = pickle.load(open('../Data/toys_remaining2.pickle', 'rb'))
    toys_copy = toys.copy()
    used_gifts = []
    not_used = []
    not_used_dict = {}
    ...
    for bag_id in xrange(50):
        ...
        for id in used_gifts:
            if id in toys:
                del toys[id]
        if len(used_gifts) == 75:
            print 'not used: ', len(not_used)
            print not_used
            for rem in not_used:
                not_used_dict[rem] = toys_copy[rem]
            pickle.dump(not_used_dict, open('../Data/toys_remaining3.pickle', 'wb'))
            break
        else:
            number_of_bags += 1
            bag = 'bag_' + str(bag_id)
...
_capture_answer().to_csv('../Submissions/submission_13A.csv', index=False)