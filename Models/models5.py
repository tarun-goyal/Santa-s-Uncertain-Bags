...
def _generate_weights():
    ...
    gloves = {}
    for j in xrange(200):
        gloves['gloves_' + str(j)] = 3.0 + np.random.rand(1)[
            0] if np.random.rand(1) < 0.3 else np.random.rand(1)[0]
    coal = {}
    for k in xrange(166):
...
_collect_filled_bags().to_csv('../Submissions/submission_15.csv', index=False)
